...
pub fn terminate(pid: u32) -> OpResult {
    ...
    if result != 0 {
        return OpResult {
            pid, success: false,
            error: Some(format!("SIGTERM failed: {}", io::Error::last_os_error())),
        };
    }
    ...
    for _ in 0..10 {
        ...
        if !process_exists(pid) {
            return OpResult { pid, success: true, error: None };
        }
    }
...
pub fn kill(pid: u32) -> OpResult {
    let result = unsafe { libc::kill(pid as i32, libc::SIGKILL) };
    if result == 0 {
        OpResult { pid, success: true, error: None }
    } else {
        OpResult { pid, success: false, error: Some(format!("SIGKILL failed: {}", io::Error::last_os_error())) }
    }
}
