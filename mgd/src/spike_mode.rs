...
pub struct SpikeSnapshot {
    pub active:  Vec<(Pid, String, SpikePhase, Kb, usize, bool)>,
    pub victims: Vec<(Pid, String, Pid)>,
}
...
impl<'a> Params<'a> {
    ...
    pub(crate) fn from_config(cfg: &'a crate::config::CompiledConfig) -> Params<'a> {
        Params {
            ...
            min_rss:                 Kb(cfg.spike_min_rss_kb),
            growth_threshold:        Kb(cfg.spike_growth_threshold_kb),
...
pub struct SpikeTracker {
    ...
    scratch_live:     HashSet<Pid>,
    scratch_rss_d:    HashMap<Pid, Kb>,
    scratch_majflt_d: HashMap<Pid, u64>,
}
...
impl SpikeTracker {
    ...
    pub(crate) fn update(
        ...
        for proc in procs {
            ...
            let growth_signal = self.scratch_rss_d.get(&proc.pid).copied().unwrap_or(Kb(0)) > p.growth_threshold;
            let majflt_signal = self.scratch_majflt_d.get(&proc.pid).copied().unwrap_or(0) > p.majflt_threshold;
            let is_behavioral = proc.rss_kb >= p.min_rss && (growth_signal || majflt_signal);
        ...
        for proc in procs {
            ...
            // Push new RSS sample
            state.samples.push_back(RssSample { rss: proc.rss_kb, taken_at: now });
            ...
            state.rss_max = state.samples.iter()
                .map(|s| s.rss)
                .max()
            ...
            if !state.has_peaked
                && state.rss_max > state.initial_rss.saturating_add(p.growth_threshold)
            {
            ...
            if state.has_peaked && !state.has_oscillated {
                let valley_floor = Kb((state.rss_max.0 as f64 * p.oscillation_drop_factor) as u64);
    ...
    pub fn spike_pids(&self) -> HashSet<Pid> {
        self.spikes.keys().copied().collect()
    }

    pub fn victim_pids(&self) -> HashSet<Pid> {
    ...
    pub fn load_and_unfreeze_victims() {
        ...
        for e in &entries {
            let pid        = Pid(e["pid"].as_u64().unwrap_or(0) as u32);
...
mod tests {
    ...
    fn p_fast() -> Params<'static> {
        Params { min_samples: 1, window_sec: 10, min_rss: Kb(0),
                 growth_threshold: Kb(100_000), ..Params::default_test() }
    }
    ...
    fn feed(pid: u32, name: &str, rss_vals: &[u64], params: &Params) -> SpikeTracker {
        ...
        for &rss in rss_vals {
            t.update(&[make_process(pid, name, rss, 0.0, 0)], Kb(u64::MAX), params);
        }
        t
    }
    ...
    fn make_victim(pid: u32, name: &str, for_spike: u32) -> SpikeVictim {
        SpikeVictim {
            pid: Pid(pid), name: name.to_string(), start_time: 0,
            frozen_for_spike_pid: Pid(for_spike), frozen_at: Instant::now(),
        }
    }
    ...
    fn t1_window_eviction() {
        let params = Params {
            window_sec: 0, min_rss: Kb(0), growth_threshold: Kb(0),
            min_samples: 1, ..p()
        };
        let mut t = feed(1, "blender", &[600_000], &params);
        t.update(&[make_process(1, "blender", 700_000, 0.0, 0)], Kb(u64::MAX), &params);
        let s = t.spikes.get(&Pid(1)).unwrap();
        assert_eq!(s.samples.len(), 1);
        assert_eq!(s.samples[0].rss, Kb(700_000));
    }
    ...
    fn t2_rss_max_after_eviction() {
        ...
        let mut t = SpikeTracker::new();
        t.spikes.insert(Pid(1), make_state_tracking(1, "blender", 0));
        // Push peak sample; window_sec=0 means it evicts as soon as time advances.
        t.update(&[make_process(1, "blender", 2_000_000, 0.0, 0)], Kb(u64::MAX), &params);
        // Next cycle: old sample evicted, rss_max recalculated from surviving samples.
        t.update(&[make_process(1, "blender", 800_000, 0.0, 0)], Kb(u64::MAX), &params);
        assert_eq!(t.spikes.get(&Pid(1)).unwrap().rss_max, Kb(800_000));
    }
    ...
    fn t3_freeze_headroom_when_insufficient() {
        ...
        let rss_max = t.spikes.get(&Pid(1)).unwrap().rss_max;
        let available = Kb((rss_max.0 as f64 * 1.0) as u64); // < headroom_factor 1.25
    ...
    fn t4_no_freeze_when_sufficient() {
        let mut t = feed(1, "cargo", &[500_000, 4_200_000, 3_500_000], &p_fast());
        let rss_max = t.spikes.get(&Pid(1)).unwrap().rss_max;
        let available = Kb((rss_max.0 as f64 * 2.0) as u64);
        let d = t.update(&[make_process(1, "cargo", 3_500_000, 0.0, 0)], available, &p_fast());
        assert!(!d.iter().any(|x| matches!(x, SpikeDecision::FreezeForHeadroom { .. })));
    }
    ...
    fn t5_throttle_spike_high_cpu() {
        ...
        let d = t.update(&[make_process(1, "blender", 3_500_000, 90.0, 0)], Kb(u64::MAX), &p_fast());
    ...
    fn t6_restore_throttle_on_cpu_drop() {
        let mut t = feed(1, "blender", &[500_000, 4_200_000, 3_500_000], &p_fast());
        t.update(&[make_process(1, "blender", 3_500_000, 90.0, 0)], Kb(u64::MAX), &p_fast());
        assert!(t.spikes.get(&Pid(1)).unwrap().cpu_throttled);
        let d = t.update(&[make_process(1, "blender", 3_500_000, 10.0, 0)], Kb(u64::MAX), &p_fast());
    ...
    fn t7_no_cpu_throttle_while_observing() {
        ...
        let params = Params { min_rss: Kb(0), cpu_threshold_pct: 80.0, ..p() };
        let mut t = SpikeTracker::new();
        let d = t.update(&[make_process(1, "blender", 600_000, 95.0, 0)], Kb(u64::MAX), &params);
        assert!(!d.iter().any(|x| matches!(x, SpikeDecision::ThrottleSpike { .. })));
    }
    ...
    fn t8_per_initiator_release() {
        let mut t = SpikeTracker::new();
        t.spikes.insert(Pid(1), make_state_tracking(1, "cargo", 1_000_000));
        t.spikes.insert(Pid(2), make_state_tracking(2, "rustc", 500_000));
        t.victims.insert(Pid(10), make_victim(10, "firefox", 1)); // frozen_for cargo
        t.victims.insert(Pid(20), make_victim(20, "spotify", 2)); // frozen_for rustc

        // cargo exits: only its victim (firefox) is released; rustc's (spotify) stays
        let v = t.on_spike_exit(Pid(1));
        assert_eq!(v.len(), 1, "cargo's victim released immediately");
        assert_eq!(v[0].pid, Pid(10));
        assert!(!t.victims.contains_key(&Pid(10)));
        assert!(t.victims.contains_key(&Pid(20)), "spotify held for rustc");

        // rustc exits (last spike): drain-all releases spotify
        let v = t.on_spike_exit(Pid(2));
        assert_eq!(v.len(), 1);
        assert_eq!(v[0].pid, Pid(20));
    ...
    fn t9_no_action_before_min_samples() {
        let params = Params { min_rss: Kb(0), growth_threshold: Kb(100_000),
                              min_samples: 6, ..p() };
        ...
        let d = t.update(&[make_process(1, "cargo", 3_500_000, 0.0, 0)], Kb(0), &params);
        assert!(d.is_empty());
    }
    ...
    fn t10_multiple_spikes_headroom_sum() {
        ...
        t.spikes.insert(Pid(1), make_state_tracking(1, "cargo",   4_000_000));
        t.spikes.insert(Pid(2), make_state_tracking(2, "blender", 3_000_000));
        let procs = vec![
            make_process(1, "cargo",   4_000_000, 0.0, 0),
            make_process(2, "blender", 3_000_000, 0.0, 0),
        ];
        // available < 4_000_000 + 3_000_000 = 7_000_000 → freeze
        let d = t.update(&procs, Kb(6_000_000), &params);
        assert!(d.iter().any(|x| matches!(x, SpikeDecision::FreezeForHeadroom { .. })));
    }
    ...
    fn t14_excluded_never_tracked() {
        ...
        let params = Params {
            min_rss: Kb(0), majflt_threshold: 0,
            exclude: vec![&excl], ..p()
        };
        let mut t = SpikeTracker::new();
        t.update(&[make_process(1, "blender", 10_000_000, 0.0, 9999)], Kb(0), &params);
    ...
    fn t15_force_include_still_requires_oscillation() {
        ...
        let params = Params {
            min_rss: Kb(u64::MAX), // would filter it out behaviorally
            min_samples: 1, growth_threshold: Kb(100_000),
            oscillation_drop_factor: 0.90,
            include: vec![&incl], ..p()
        };
        ...
        let d = t.update(&[make_process(1, "plasmashell", 400_000, 0.0, 0)], Kb(0), &params);
        assert!(d.is_empty());
    }

    // T16 ─ needed == exact deficit
    #[test]
    fn t16_needed_kb_equals_deficit() {
        let mut t = feed(1, "cargo", &[500_000, 4_200_000, 3_500_000], &p_fast());
        let rss_max = t.spikes.get(&Pid(1)).unwrap().rss_max;
        let required = Kb((rss_max.0 as f64 * 1.25) as u64);
        let available = required.saturating_sub(Kb(500_000));
        let d = t.update(&[make_process(1, "cargo", 3_500_000, 0.0, 0)], available, &p_fast());
    ...
    fn t17_majflt_delta_from_cache() {
        let params = Params {
            min_rss: Kb(0),
            growth_threshold: Kb(u64::MAX), // disable growth signal
            majflt_threshold: 100,
            min_samples: 1, ..p()
        };
        let mut t = SpikeTracker::new();
        // Cycle 1: cumulative majflt=50, no prev → delta=0, below threshold → not tracked
        t.update(&[make_process(1, "ld", 600_000, 0.0, 50)], Kb(u64::MAX), &params);
        assert!(t.spikes.is_empty(), "delta=0 on first observation, should not track");
        assert_eq!(*t.prev_majflt.get(&Pid(1)).unwrap(), 50, "prev_majflt must be updated");
